...
employee_blueprint = Blueprint("Employee", url_prefix="/employee")
...
async def register_user(request: Request):
    try:
        ...
        email = body["email"]
        result = ModelGetter.SQL.get_model().employee.add_new_user(username, password, email)
...
async def login_app(request: Request):
    try:
        ...
        username = body["username"]
        password = body["password"]
        user_data = ModelGetter.SQL.get_model().employee.get_employee_by_username_password(username, password)
