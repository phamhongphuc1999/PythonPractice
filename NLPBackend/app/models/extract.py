...
from app.models.attenion import prob_normalize
from app.models.rnn import lstm_encoder, MultiLayerLSTMCells
from app.models.util import sequence_mean, len_mask
...
class ExtractSumm(nn.Module):
    ...
    def __init__(self, vocab_size, emb_dim, conv_hidden, lstm_hidden, lstm_layer, bidirectional, dropout=0.0):
        ...
        self._art_enc = LSTMEncoder(
            3 * conv_hidden, lstm_hidden, lstm_layer, dropout=dropout, bidirectional=bidirectional
        )
...
class LSTMPointerNet(nn.Module):
    ...
    def _prepare(self, attn_mem):
        ...
        size = (n_l, bs, d)
        lstm_states = (
            self._init_h.unsqueeze(1).expand(*size).contiguous(),
