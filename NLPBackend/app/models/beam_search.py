...
class _Hypothesis(object):
    ...
    def __lt__(self, other):
        return other.logprob / len(other.sequence) < self.logprob / len(self.sequence)
...
def create_beam(tok, lp, hists):
    ...
    k = tok.size(0)
    return [_Hypothesis([tok[i].item()], lp[i].item(), hists) for i in range(k)]
...
def _clean_beam(finished, beam, end_tok, beam_size, remove_tri=True):
    ...
    new_beam = []
    for h in sorted(beam, reverse=True, key=lambda h: h.logprob / len(h.sequence)):
        if remove_tri and _has_repeat_tri(h.sequence):
            h.logprob = -1e9
        if h.sequence[-1] == end_tok:
            finished_hyp = _Hypothesis(h.sequence[:-1], h.logprob, h.hists, h.attns)
    ...
    finished = sorted(finished, reverse=True, key=lambda h: h.logprob / len(h.sequence))
