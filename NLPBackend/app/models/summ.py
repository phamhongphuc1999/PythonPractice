...
from app.models.rnn import MultiLayerLSTMCells, lstm_encoder
from app.models.util import len_mask, sequence_mean
...
class Seq2SeqSumm(nn.Module):
    def __init__(
        ...
        self._enc_lstm = nn.LSTM(
            emb_dim, n_hidden, n_layer, bidirectional=bidirectional, dropout=dropout
        )
        ...
        self._decoder = AttentionalLSTMDecoder(
            self._embedding, self._dec_lstm, self._attn_wq, self._projection
        )
    ...
    def encode(self, article, art_lens=None):
        ...
        init_attn_out = self._projection(
            torch.cat([init_h[-1], sequence_mean(attention, art_lens, dim=1)], dim=1)
        )
