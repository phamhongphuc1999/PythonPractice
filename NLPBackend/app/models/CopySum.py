...
from app.models.attenion import step_attention
from app.models.summ import Seq2SeqSumm, AttentionalLSTMDecoder
from app.models.util import len_mask
from torch.nn import functional as F

from app.models import beam_search as bs
...
class _CopyLinear(nn.Module):
    ...
    def forward(self, context, state, input_):
        output = (
            torch.matmul(context, self._v_c.unsqueeze(1))
...
class CopySumm(Seq2SeqSumm):
    ...
    def batched_beamsearch(
        ...
        for t in range(max_len):
            ...
            states = (
                (
                    torch.stack([h for (h, _), _ in all_states], dim=2),
            ...
            for i, (beam, finished) in enumerate(zip(all_beams, finished_beams)):
                ...
                if len(finished) >= beam_size:
                    ...
                    (attention, mask, extend_art, extend_vsize) = all_attention
...
class CopyLSTMDecoder(AttentionalLSTMDecoder):
    ...
    def topk_step(self, tok, states, attention, k):
        ...
        nl, _, _, d = h.size()
        beam, batch = tok.size()
        lstm_in_beamable = torch.cat([self._embedding(tok), prev_out], dim=-1)
